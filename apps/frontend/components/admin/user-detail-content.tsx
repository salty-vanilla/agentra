...
type Props = {
  user: AdminUser;
  onClose: () => void;
  onUserUpdated?: ((updated: AdminUser) => void) | undefined;
};
...
/**
 * The shared body of the Admin user detail view. Container-agnostic: it is
 * rendered inside a full-screen Sheet (Compact), a modal Drawer (Medium), or a
 * non-modal side panel (Expanded) via {@link AdminResponsiveDetail}. The detail
 * markup lives here exactly once so the three containers never diverge.
 */
export function UserDetailContent({ user, onClose, onUserUpdated }: Props) {
  ...
  return (
    <>
      <div className="px-4 space-y-1">
        ...
        <p className="text-xs font-medium text-muted-foreground pt-4 pb-1">識別子</p>
        <dl>
          <DetailRow
            label="User ID"
            value={
