...
import type { ProcessFlowWithImpactInput } from "#src/strategy/strategy-input-schemas.js";
...
export const processFlowWithImpactStrategy: LayoutStrategy = {
  ...
  match(ctx: LayoutContext): boolean {
    if (ctx.strategyInput != null) {
      const r = readStrategyInput<ProcessFlowWithImpactInput>({ strategyId: "process-flow-with-impact", strategyInput: ctx.strategyInput });
      if (r.ok) return true;
    }
  ...
  layout(ctx: LayoutContext): LayoutResult {
    const sir = readStrategyInput<ProcessFlowWithImpactInput>({ strategyId: "process-flow-with-impact", strategyInput: ctx.strategyInput });
    if (sir.ok && sir.input) {
      const inp = sir.input;
      const syntheticBlocks: ContentBlock[] = [];
      for (const [i, step] of inp.steps.entries()) {
        const prefix = step.bottleneck ? "⚠ BOTTLENECK: " : "";
        const text = `${prefix}${step.label}${step.description ? "\n" + step.description : ""}`;
        syntheticBlocks.push({ id: `si-paragraph-${i}`, type: "paragraph", text });
        if (step.impact && step.bottleneck) {
          syntheticBlocks.push({ id: `si-metric-${i}`, type: "metric", label: step.label, value: step.impact });
        }
      }
      if (inp.keyTakeaway) {
