...
export function buildAuthoringPrompt(input: PresentationAuthorInput): string {
  ...
  const parts: string[] = [
    ...
    '- Import helpers: `const { autoFontSize, calcTextBox, imageSizingCrop, imageSizingContain, warnIfSlideHasOverlaps, warnIfSlideElementsOutOfBounds } = require("./helpers/pptxgenjs_helpers");`',
    ...
    '- Use `autoFontSize` or `calcTextBox` for dense text to ensure it fits.',
    '- Use `imageSizingCrop` / `imageSizingContain` when placing images.',
